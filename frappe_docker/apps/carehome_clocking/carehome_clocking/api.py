...
def kiosk_login(device_id, pin):
    ...
    try:
        ...
        if not device.verify_pin(pin):
            frappe.log_error(f"Invalid PIN attempt for device {device_id}")
        ...
        frappe.log_error(f"Kiosk login error: {str(e)}")
...
def kiosk_heartbeat(device_id, token, queue_size=0):
    ...
    Args:
        ...
        token: Session token
        queue_size: Number of pending offline punches
    """
    try:
        device = frappe.get_doc('Kiosk Device', device_id)
...
def initiate_enrollment(employee_id):
    ...
    Returns:
        dict: QR code image (base64), enrollment token, expiry
    """
    ...
    try:
        ...
        frappe.log_error(f"Enrollment error: {str(e)}")
        return {'success': False, 'error': str(e)}
...
def complete_enrollment(employee_id, enrollment_token):
    ...
    Called after mobile app stores the TOTP secret
...
def validate_token(employee_id, totp_code, device_id=None):
    ...
    Args:
        ...
        device_id: Optional kiosk device ID
    
    Returns:
        dict: Validation result and checkin info
    """
    try:
        # Get enrollment
        enrollment_name = frappe.db.get_value(
...
def get_employee_state(employee_id, device_id=None):
    ...
    Returns:
        dict: Last punch info and current state
    """
    try:
        employee = frappe.get_doc('Employee', employee_id)
...
def sync_offline_queue(punches, device_id):
    ...
    Args:
        punches: JSON array of punch records
        device_id: Kiosk device ID
    ...
    try:
        ...
        frappe.log_error(f"Offline sync error: {str(e)}")
        return {'success': False, 'error': str(e)}
...
@frappe.whitelist()
def create_swap_request(requester_shift, target_employee, swap_date, notes=None):
    """
    Create a shift swap request
    
    Args:
        requester_shift: Shift assignment ID
        target_employee: Target employee ID
        swap_date: Date of swap
        notes: Optional notes
    
    Returns:
        dict: Created swap request
    """
    try:
...
def respond_to_swap(swap_id, action, notes=None):
    ...
    Args:
        swap_id: Shift Swap Request ID
        action: 'accept' or 'decline'
        notes: Optional notes
    
    Returns:
        dict: Updated swap status
    """
    try:
...
def get_live_status(care_home=None):
    ...
    Returns:
        dict: List of clocked-in employees
    """
    frappe.only_for(['HR Manager', 'Care Home Manager', 'System Manager'])
...
def get_sync_health():
    ...
    try:
        devices = frappe.get_all(
            'Kiosk Device',
            filters={'is_active': 1},
            fields=['name', 'device_name', 'care_home', 'last_seen_at', 'offline_queue_size']
        )
        ...
        return {
            ...
            'total_pending_syncs': sum(d['offline_queue_size'] or 0 for d in health_data)
        }
...
def get_my_status():
    ...
    Returns:
        dict: Today's status, recent punches, weekly hours
    """
    try:
        user_employee = frappe.db.get_value(
            'Employee',
...
def get_my_swap_requests():
    ...
    Returns:
        dict: Sent and received swap requests
    """
    try:
        user_employee = frappe.db.get_value(
            'Employee',
...
def get_care_home_employees(care_home, device_id, token):
    ...
    Args:
        care_home: Care Home ID
        device_id: Kiosk device ID
        token: Session token
    
    Returns:
        dict: Employee IDs and names for the care home
    """
    try:
