...
const LateArrivalsReport = () => {
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      ...
      console.error('Failed to fetch late arrivals report:', err);
  ...
  const getJobTitleClass = (jobTitle) => {
    const classes = {
  ...
  const getLatenessSeverity = (minutes) => {
  ...
  return (
    <div className="max-w-6xl mx-auto" data-testid="late-arrivals-report">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          onClick={exportToCSV}
          disabled={!reportData?.late_arrivals?.length}
      ...
      {error ? (
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-6 text-center">
          ...
          <button
            onClick={fetchReport}
            className="mt-4 px-4 py-2 bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300 rounded-lg hover:bg-red-200"
          >
            Try Again
          </button>
        </div>
        ...
        <>
          ...
          {/* Repeat Offenders */}
          {reportData?.summary?.repeat_offenders?.length > 0 && (
            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-4 mb-6" data-testid="repeat-offenders">
              ...
              <div className="flex flex-wrap gap-2">
                {reportData.summary.repeat_offenders.map((offender, idx) => (
                  <div
                    key={idx}
                    className="px-3 py-1.5 bg-amber-100 dark:bg-amber-900/40 text-amber-800 dark:text-amber-200 rounded-full text-sm flex items-center gap-2"
                  >
                    <User size={14} />
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 border border-gray-200 dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="px-3 py-2 border border-gray-200 dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500"
                  data-testid="filter-job-title"
                >
                  <option value="all">All Job Titles</option>
                  {uniqueJobTitles.map(jt => (
                    <option key={jt} value={jt}>{getJobTitleDisplay(jt)}</option>
                  ))}
                </select>
              </div>
            </div>
            ...
            {filteredArrivals.length === 0 ? (
              <div className="p-12 text-center">
                ...
                <p className="text-gray-500 dark:text-gray-400">
                  {searchTerm || filterJobTitle !== 'all' 
                    ? 'No results match your filters' 
                    : `Great news! No late arrivals recorded for ${monthNames[month - 1]} ${year}`}
                </p>
              </div>
            ) : (
                    ...
                    {filteredArrivals.map((arrival, idx) => (
                      <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                        <td className="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">
                          {new Date(arrival.date).toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'short' })}
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${getJobTitleClass(arrival.job_title)}`}>
                            {getJobTitleDisplay(arrival.job_title)}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${getLatenessSeverity(arrival.minutes_late)}`}>
                            {arrival.minutes_late} min
                          </span>
                        </td>
