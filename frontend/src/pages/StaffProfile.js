...
const StaffProfile = () => {
  ...
  const fetchAllData = async () => {
    setLoading(true);
    try {
      const [profileRes, shiftsRes, leaveRes, dayRes, swapRes, colleaguesRes] = await Promise.all([
      ...
      setShiftSwaps(swapRes.data.shift_swaps || []);
  ...
  const getStatusBadge = (status) => {
    const styles = {
      ...
      scheduled: 'frappe-badge-primary',
      completed: 'frappe-badge-gray'
    };
    return <span className={`frappe-badge ${styles[status] || 'frappe-badge-gray'}`}>{status}</span>;
  };
        ...
        <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 p-6">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="flex gap-3 mt-6">
              <button
                type="button"
                onClick={() => setShowDayRequestModal(false)}
                className="frappe-btn frappe-btn-secondary flex-1"
              >
                Cancel
              </button>
              <button
                ...
                disabled={submitting}
                className="frappe-btn frappe-btn-primary flex-1"
                data-testid="submit-day-request-btn"
              >
                {submitting ? 'Submitting...' : 'Submit Request'}
              </button>
            </div>
          ...
          {selectedShift && (
            <div className="p-4 bg-blue-50 rounded-lg mb-4">
              <p className="text-sm text-blue-700">
                <strong>Shift:</strong> {formatDate(selectedShift.shift_date)}
              </p>
              <p className="text-sm text-blue-700">
                <strong>Time:</strong> {selectedShift.start_time} - {selectedShift.end_time}
              </p>
            </div>
          )}
          <form onSubmit={handleSubmit}>
            <div className="space-y-4">
              ...
              <p className="text-sm text-gray-500">
                Your swap request will be visible to all staff members who can accept it.
              </p>
            </div>
            <div className="flex gap-3 mt-6">
              <button
                type="button"
                onClick={() => { setShowSwapModal(false); setSelectedShift(null); }}
                className="frappe-btn frappe-btn-secondary flex-1"
              >
                Cancel
              </button>
  ...
  const handleAcceptSwap = async (swapId) => {
    try {
      await axios.post(`${API}/shift-swaps/${swapId}/accept`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchAllData();
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <header className="frappe-header justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={goToClockScreen}
            className="frappe-btn frappe-btn-primary"
            data-testid="go-to-clock-btn"
          >
            <Clock size={16} />
      ...
      <aside className="frappe-sidebar">
        ...
        <button onClick={() => setActiveTab('swaps')} className={`frappe-sidebar-item w-full text-left ${activeTab === 'swaps' ? 'active' : ''}`}>
          ...
          {shiftSwaps.filter(s => s.status === 'open' && s.requester_id !== user?.id).length > 0 && (
            <span className="ml-auto bg-red-500 text-white text-xs rounded-full px-2 py-0.5">
              {shiftSwaps.filter(s => s.status === 'open' && s.requester_id !== user?.id).length}
            </span>
          )}
        </button>
          ...
          <>
            ...
            <div className="grid grid-cols-4 gap-4 mb-6">
              ...
              <div className="frappe-stat-card">
                ...
                <div className="frappe-stat-value text-orange-600">
                  {profile.pending_counts.leave_requests + profile.pending_counts.day_requests}
                </div>
              </div>
              <div className="frappe-stat-card">
        ...
        {activeTab === 'rota' && (
          <>
            <div className="flex items-center justify-between mb-6">
              <h1 className="frappe-page-title mb-0">My Rota</h1>
              <button onClick={fetchAllData} className="frappe-btn frappe-btn-secondary">
                <RefreshCcw size={16} />
                <span>Refresh</span>
              </button>
            </div>
            ...
            <div className="frappe-card">
              <div className="frappe-card-header">Next 4 Weeks</div>
                ...
                <table className="frappe-table">
                  ...
                  <tbody>
                    {shifts.length === 0 ? (
                      ...
                      shifts.map((shift) => (
                        <tr key={shift.id} data-testid={`shift-row-${shift.id}`}>
                          <td className="font-medium">{formatDate(shift.shift_date)}</td>
                          <td>{shift.start_time}</td>
                          <td>{shift.end_time}</td>
                          <td>
                            <span className="frappe-badge frappe-badge-gray">{shift.shift_type}</span>
                          </td>
                          ...
                          <td>
                            {shift.status === 'scheduled' && (
                              <button
                                ...
                                data-testid={`swap-shift-btn-${shift.id}`}
                              >
                                <RefreshCcw size={14} />
                                Swap
                              </button>
        ...
        {activeTab === 'swaps' && (
          <>
            ...
            <div className="frappe-card mb-6">
              ...
              <div className="frappe-card-content">
                {shiftSwaps.filter(s => s.status === 'open' && s.requester_id !== user?.id).length === 0 ? (
                  <p className="text-gray-500 text-center py-4">No shift swaps available</p>
                ) : (
                  <div className="space-y-3">
                    {shiftSwaps.filter(s => s.status === 'open' && s.requester_id !== user?.id).map((swap) => (
            ...
            <div className="frappe-card">
              ...
              <div className="frappe-card-content">
                {shiftSwaps.filter(s => s.requester_id === user?.id).length === 0 ? (
                  <p className="text-gray-500 text-center py-4">You haven't posted any swap requests</p>
                ) : (
                  <div className="space-y-3">
                    {shiftSwaps.filter(s => s.requester_id === user?.id).map((swap) => (
                      <div key={swap.id} className="p-4 border border-gray-200 rounded-lg flex items-center justify-between">
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            {swap.accepted_by_name && (
                              <span className="text-sm text-green-600">Accepted by {swap.accepted_by_name}</span>
                            )}
                          </div>
                        </div>
                        {swap.status === 'open' && (
                          <button
                            ...
                            className="frappe-btn frappe-btn-secondary text-red-600"
                            data-testid={`cancel-swap-btn-${swap.id}`}
                          >
                            <Trash2 size={14} />
                            Cancel
                          </button>
        ...
        {activeTab === 'leave' && (
          <>
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setShowLeaveModal(true)} className="frappe-btn frappe-btn-primary" data-testid="request-leave-btn">
                <Plus size={16} />
                          ...
                          <td>
                            {req.status === 'pending' && (
                              <button
                                ...
                                className="frappe-btn frappe-btn-secondary text-xs py-1 px-2 text-red-600"
                              >
                                Cancel
                              </button>
                            )}
                  ...
                  <tbody>
                    {leaveRequests.filter(r => ['sick', 'compassionate', 'unpaid'].includes(r.leave_type)).length === 0 ? (
                      ...
                      leaveRequests.filter(r => ['sick', 'compassionate', 'unpaid'].includes(r.leave_type)).map((req) => (
                        <tr key={req.id}>
                          <td>
                            <span className={`frappe-badge ${req.leave_type === 'sick' ? 'frappe-badge-error' : 'frappe-badge-warning'}`}>
                              {req.leave_type}
                            </span>
                          </td>
                          <td>{formatDate(req.start_date)}</td>
        ...
        {activeTab === 'dayRequests' && (
          <>
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setShowDayRequestModal(true)} className="frappe-btn frappe-btn-primary" data-testid="request-day-btn">
                <Plus size={16} />
                  ...
                  <tbody>
                    {dayRequests.length === 0 ? (
                      ...
                      dayRequests.map((req) => (
                        <tr key={req.id}>
                          <td>
                            <span className={`frappe-badge ${req.request_type === 'day_on' ? 'frappe-badge-success' : 'frappe-badge-primary'}`}>
                              {req.request_type === 'day_on' ? 'Day On' : 'Day Off'}
                            </span>
                          </td>
